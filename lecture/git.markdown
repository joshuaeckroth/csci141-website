...
- **repository**: git stores data about versions in a repository. If
  you put your files in a folder called, for example, `csci141-homework-1`, and
  next run the `git init` command (described below), then the
  repository will be created in a special hidden folder `.git` inside
  the `csci141-homework-1` folder. You can look in this folder if you want;
...
X/Windows, assuming you have installed Git.
    ...
    mkdir csci141-homework-1              # create a new directory
    cd csci141-homework-1                 # go into that directory
    ...
    git remote add origin git@bitbucket.org:joshuaeckroth/csci141-demo.git
